...
function GradeTab() {
  ...
  const handleSave = () => {
    ...
    // TODO: 백엔드 연결 시 여기에 API 호출 추가
    // await fetch('/api/grades', { method:'POST', body: JSON.stringify(newEntry) })
            ...
            {showAdd ? (
              <div style={{ padding:'12px 14px', borderRadius:10, border:'1.5px solid var(--color-primary)', background:'var(--color-primary-light)', display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <input value={newTask.task} onChange={e => setNewTask(p=>({...p, task:e.target.value}))}
                  ...
                  style={{ width:'100%', padding:'9px 12px', borderRadius:8, border:'1.5px solid var(--color-border)', background:'var(--color-surface)', fontSize:13, fontFamily:'inherit', outline:'none' }} />
                <div style={{ display:'flex', gap:8 }}>
