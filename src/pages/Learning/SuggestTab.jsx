...
const TENDENCY_INFO = {
  'VISUAL':     { emoji: '👁️', label: '시각형',  color: '#1A56DB', description: '이미지, 도표, 마인드맵 활용에 강해요' },
  'AUDITORY':   { emoji: '🎧', label: '청각형',  color: '#9B59B6', description: '소리내어 읽기, 토론, 강의 듣기에 강해요' },
  'KINESTHETIC':{ emoji: '🤸', label: '행동형',  color: '#FF6B35', description: '직접 손으로 쓰고 움직이며 학습할 때 효과적이에요' },
}

const SUBJECT_ID_BY_NAME = { '국어':1, '수학':2, '영어':3, '한국사':4, '사회탐구':5, '과학탐구':6 }
...
function PeerContentCard({ item }) {
  const SUBJECT_COLORS_LOCAL = { 국어:'#1A56DB', 수학:'#FF6B35', 영어:'#00C49A', 사회:'#9B59B6', 과학:'#FFB800' }
  ...
  const getTypeIcon = (t) => {
    ...
    if (u.includes('MOCK') || u.includes('EXAM')) return '📝'
    if (u.includes('CHALLENGE')) return '🏆'
    if (u.includes('PRACTICE')) return '✏️'
    if (u.includes('VIDEO')) return '🎥'
    return '📚'
  }
  return (
    <div style={{ padding:'12px 14px', borderRadius:10, background:'var(--color-surface-2)', border:`1px solid ${color}30`, display:'flex', alignItems:'center', gap:12, cursor:'pointer' }}>
      <span style={{ fontSize:24, flexShrink:0 }}>{getTypeIcon(type)}</span>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
          {subject && <span style={{ padding:'2px 7px', borderRadius:6, background:color+'18', color, fontSize:9, fontWeight:700 }}>{subject}</span>}
          <p style={{ fontSize:13, fontWeight:700, color:'var(--color-text-primary)' }}>{title}</p>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10, fontSize:10, color:'var(--color-text-muted)' }}>
          {peerCount != null && <span>👥 {peerCount}명이 푸는 중</span>}
          {avgImprovement != null && <span style={{ color:'var(--color-success)', fontWeight:700 }}>▲ 평균 {avgImprovement}점 상승</span>}
          {avgScore != null && !avgImprovement && <span>평균 {avgScore}점</span>}
        </div>
      </div>
...
export default function SuggestTab({ studentId: propStudentId, selectedChild }) {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      const map = {}
      entries.forEach(([subject, data]) => { if (data) map[subject] = data })
      setStudyMethods(map)
    ...
    <div>
      ...
      <div style={{ margin:'12px 16px 0' }}>
        <div style={{ background:'var(--color-surface)', borderRadius:16, border:'1px solid var(--color-border)', padding:16 }}>
          ...
          <p style={{ fontSize:12, color:'var(--color-text-muted)', marginBottom:14 }}>AI가 분석한 개인 맞춤 학습 자료</p>
          {materialsLoading ? (
            <div style={{ padding:'40px 0', textAlign:'center', color:'var(--color-text-muted)', fontSize:13 }}>추천 자료 분석 중...</div>
          ) : materials.length > 0 ? (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {(materialsExpanded ? materials : materials.slice(0, 3)).map((m, i) => {
                ...
                return (
                  <div key={m.materialId || i} style={{ display:'flex', alignItems:'center', gap:14, padding:'13px 14px', borderRadius:12, background:'var(--color-surface-2)', border:'1px solid var(--color-border)' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ fontSize:13, fontWeight:700, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{m.title}</p>
                      <div style={{ display:'flex', gap:6, marginTop:4, flexWrap:'wrap' }}>
                        <span style={{ fontSize:10, fontWeight:600, padding:'2px 7px', borderRadius:20, background:'var(--color-primary-light)', color:'var(--color-primary)' }}>{icon.label}</span>
                      </div>
                      {m.matchReason && <p style={{ fontSize:11, color:'var(--color-text-muted)', marginTop:6, lineHeight:1.5 }}>💡 {m.matchReason}</p>}
                    </div>
                  </div>
                )
              })}
              {materials.length > 3 && (
                <button onClick={() => setMaterialsExpanded(prev => !prev)}
                  style={{ width:'100%', padding:'10px', borderRadius:10, border:'1px solid var(--color-border)', background:'var(--color-surface-2)', fontSize:12, fontWeight:600, fontFamily:'inherit', cursor:'pointer', color:'var(--color-text-muted)', marginTop:4 }}>
                  {materialsExpanded ? '▲ 접기' : `▼ 더 보기 (+${materials.length - 3}개)`}
                </button>
              )}
            </div>
          ) : (
            <div style={{ padding:'32px 16px', textAlign:'center', background:'var(--color-surface-2)', borderRadius:12, border:'1px dashed var(--color-border)' }}>
              ...
              <p style={{ fontSize:13, color:'var(--color-text-muted)' }}>아직 추천 자료가 없어요</p>
              <p style={{ fontSize:11, color:'var(--color-text-muted)', marginTop:4 }}>학습 기록이 쌓이면 맞춤 자료를 추천해드릴게요</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ margin:'12px 16px 0' }}>
        <div style={{ background:'var(--color-surface)', borderRadius:16, border:'1px solid var(--color-border)', padding:16 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            <p style={{ fontSize:14, fontWeight:700 }}>👥 또래가 푸는 콘텐츠</p>
            {peerContent?.peerLevel && (
              <span style={{ padding:'3px 10px', borderRadius:20, background:'var(--color-primary-light)', color:'var(--color-primary)', fontSize:10, fontWeight:700 }}>{peerContent.peerLevel}</span>
            )}
          </div>
          <p style={{ fontSize:11, color:'var(--color-text-muted)', marginBottom:12, lineHeight:1.5 }}>💡 비슷한 성적대 학생들이 함께 풀고 점수가 향상된 콘텐츠예요</p>
          {peerContentLoading ? (
            <div style={{ padding:'20px', textAlign:'center', color:'var(--color-text-muted)', fontSize:12 }}>또래 콘텐츠 분석 중...</div>
            ...
            <div style={{ padding:'32px 16px', textAlign:'center', background:'var(--color-surface-2)', borderRadius:12, border:'1px dashed var(--color-border)' }}>
              <p style={{ fontSize:32, marginBottom:8 }}>👥</p>
              <p style={{ fontSize:13, color:'var(--color-text-muted)' }}>아직 또래 콘텐츠 데이터가 없어요</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ margin:'12px 16px 0' }}>
        <div style={{ background:'var(--color-surface)', borderRadius:16, border:'1px solid var(--color-border)', padding:16 }}>
          ...
          {peerPathsLoading ? (
            <div style={{ padding:'40px 0', textAlign:'center', color:'var(--color-text-muted)', fontSize:13 }}>선배 학습 경로 분석 중...</div>
          ) : peerPaths.length > 0 ? (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {peerPaths.map(p => {
                ...
                return (
                  <div key={p.subject} style={{ padding:'12px 14px', borderRadius:12, background:'var(--color-surface-2)', border:'1px solid var(--color-border)' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                      ...
                      {p.similarStudentsCount != null && <span style={{ fontSize:11, color:'var(--color-text-muted)' }}>{p.similarStudentsCount}명 분석</span>}
                    </div>
                    {p.avgImprovement != null && (
                      <div style={{ padding:'10px 12px', borderRadius:8, background:'#D1FAF015', border:'1px solid #00C49A30', marginBottom:8, textAlign:'center' }}>
                        ...
                        <p style={{ fontSize:18, fontWeight:800, color:'var(--color-success)' }}>▲ {p.avgImprovement}{typeof p.avgImprovement === 'number' ? '점' : ''}</p>
                      </div>
                    )}
                    {p.studyStrategyPattern && (
                      <div style={{ padding:'8px 10px', borderRadius:8, background:'var(--color-primary-light)', border:'1px solid #1A56DB20', marginBottom:6 }}>
                        ...
                        <p style={{ fontSize:11, color:'var(--color-text-primary)', lineHeight:1.5 }}>{p.studyStrategyPattern}</p>
                      </div>
                    )}
                    {p.keyInsights && (
                      <div style={{ padding:'8px 10px', borderRadius:8, background:'#FFF8E0', border:'1px solid #FFB80030' }}>
                        ...
                        <p style={{ fontSize:11, color:'var(--color-text-primary)', lineHeight:1.5 }}>{p.keyInsights}</p>
                      </div>
                    )}
                  </div>
                )
            ...
            <div style={{ padding:'32px 16px', textAlign:'center', background:'var(--color-surface-2)', borderRadius:12, border:'1px dashed var(--color-border)' }}>
              <p style={{ fontSize:32, marginBottom:8 }}>📊</p>
              <p style={{ fontSize:13, color:'var(--color-text-muted)', marginBottom:4 }}>아직 분석할 데이터가 부족해요</p>
              <p style={{ fontSize:11, color:'var(--color-text-muted)' }}>학습 기록이 쌓이면 비슷한 선배들의 성공 경로를 보여드릴게요</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ margin:'12px 16px 16px' }}>
        <div style={{ background:'var(--color-surface)', borderRadius:16, border:'1px solid var(--color-border)', padding:16 }}>
          ...
          <p style={{ fontSize:12, color:'var(--color-text-muted)', marginBottom:14 }}>실제 학습 기록 분석 기반</p>
          {patternLoading ? (
            <div style={{ padding:'40px 0', textAlign:'center', color:'var(--color-text-muted)', fontSize:13 }}>학습 패턴 분석 중...</div>
            ...
            <>
                ...
                <div style={{ padding:'12px 14px', borderRadius:12, background:'linear-gradient(90deg, #FFF8E0, #FFE9D6)', border:'1px solid #FFB80040', marginBottom:14 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize:16, fontWeight:800, color:'#8A6500', marginTop:2 }}>{studyPattern.goldenTime}</p>
                    </div>
                  </div>
                </div>
              )}
              <p style={{ fontSize:12, fontWeight:700, color:'var(--color-text-secondary)', marginBottom:8 }}>📊 과목별 학습 비중</p>
              <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
                {(() => {
                  ...
                  return entries.map(([subject, minutes]) => {
                    ...
                    return (
                      <div key={subject} style={{ flex:'1 1 30%', minWidth:90, textAlign:'center', padding:'12px 8px', borderRadius:12, background:isTop ? color+'18' : 'var(--color-surface-2)', border:`1.5px solid ${isTop ? color : 'var(--color-border)'}` }}>
              ...
              {studyPattern.studyBalanceSummary && (
                <div style={{ padding:'10px 12px', borderRadius:10, background:'var(--color-primary-light)', border:'1px solid #1A56DB20', marginBottom:14 }}>
                  <p style={{ fontSize:11, color:'var(--color-primary)', lineHeight:1.6, fontWeight:600 }}>💡 {studyPattern.studyBalanceSummary}</p>
                </div>
              )}
              {(() => {
                const subjects = Object.keys(studyMethods)
                if (subjects.length === 0) return null
                const topSubject = Object.entries(studyPattern.subjectStudyMinutes).sort((a, b) => b[1] - a[1])[0]?.[0]
                ...
                return (
                  <div style={{ padding:'16px', borderRadius:14, background:`linear-gradient(135deg, ${info.color}15, ${info.color}08)`, border:`1.5px solid ${info.color}40`, marginBottom:14 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                      <span style={{ fontSize:36 }}>{info.emoji}</span>
                      <div>
                        <p style={{ fontSize:11, fontWeight:600, color:info.color, marginBottom:2 }}>나의 학습 성향 ({topSubject} 기준)</p>
                        <p style={{ fontSize:20, fontWeight:800, color:info.color }}>{info.label}</p>
                        <p style={{ fontSize:11, color:'var(--color-text-muted)', marginTop:2 }}>{info.description}</p>
                      </div>
                    </div>
                    {mainMethod.studyGuide && (
                      <div style={{ padding:'10px 12px', borderRadius:10, background:'rgba(255,255,255,0.6)', marginBottom:6 }}>
                        ...
                        <p style={{ fontSize:12, color:'var(--color-text-primary)', lineHeight:1.6 }}>{mainMethod.studyGuide}</p>
                      </div>
                    )}
                    {mainMethod.recommendedApproach && (
                      <div style={{ padding:'10px 12px', borderRadius:10, background:'rgba(255,255,255,0.6)' }}>
                        ...
                        <p style={{ fontSize:12, color:'var(--color-text-primary)', lineHeight:1.6 }}>{mainMethod.recommendedApproach}</p>
                      </div>
                    )}
                  </div>
                )
              })()}
              <p style={{ fontSize:12, fontWeight:700, color:'var(--color-text-secondary)', marginBottom:8 }}>📚 과목별 학습 가이드</p>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {Object.keys(studyPattern.subjectStudyMinutes).map(subject => {
                  ...
                  if (apiData?.studyGuide) {
                    ...
                    return (
                      <div key={subject} style={{ padding:'12px 14px', borderRadius:10, background:'var(--color-surface-2)', border:`1px solid ${info.color}30` }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                          <span style={{ fontSize:18 }}>{info.emoji}</span>
                          <span style={{ fontSize:13, fontWeight:700, color:SUBJECT_COLORS[subject] || 'var(--color-primary)' }}>{subject}</span>
                          <span style={{ fontSize:10, fontWeight:600, padding:'2px 8px', borderRadius:20, background:info.color+'18', color:info.color }}>{info.label}</span>
                        </div>
                        <p style={{ fontSize:12, color:'var(--color-text-secondary)', lineHeight:1.5, marginBottom:4 }}>{apiData.studyGuide}</p>
                        {apiData.recommendedApproach && (
                          <p style={{ fontSize:11, color:'var(--color-text-muted)', lineHeight:1.5, marginTop:6, paddingTop:6, borderTop:'1px dashed var(--color-border)' }}>💡 {apiData.recommendedApproach}</p>
                        )}
                      </div>
                    )
                  }
            ...
            <div style={{ padding:'32px 16px', textAlign:'center', background:'var(--color-surface-2)', borderRadius:12, border:'1px dashed var(--color-border)' }}>
              <p style={{ fontSize:32, marginBottom:8 }}>📭</p>
              <p style={{ fontSize:13, color:'var(--color-text-muted)', marginBottom:4 }}>아직 분석할 학습 기록이 없어요</p>
              <p style={{ fontSize:11, color:'var(--color-text-muted)' }}>학습을 시작하면 맞춤 분석을 받을 수 있어요</p>
            </div>
          )}
        </div>
      </div>
