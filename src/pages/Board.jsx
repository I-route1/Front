...
export default function Board() {
  ...
  return (
    <div>
      <section style={{ padding: '20px 20px 14px', background: 'var(--color-surface)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          ...
          <button
            ...
            style={{ padding: '10px 14px', borderRadius: 12, background: 'var(--color-primary)', color: 'white', fontSize: 13, fontWeight: 700, boxShadow: '0 4px 12px rgba(26,86,219,0.25)', flexShrink: 0 }}
          >
            글쓰기
          </button>
        </div>
      ...
      {noticeMessage && (
        <div style={{ margin: '12px 16px 0', padding: '10px 12px', borderRadius: 12, background: '#fff7ed', color: '#c2410c', fontSize: 12, fontWeight: 700, lineHeight: 1.5 }}>
          {noticeMessage}
        </div>
      )}
        ...
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state__desc">
              {activeTab === '즐겨찾기' ? '즐겨찾기한 게시글이 없습니다.' : '검색어를 변경하거나 새 글을 작성해 보세요.'}
            </p>
          </div>
        ) : (
